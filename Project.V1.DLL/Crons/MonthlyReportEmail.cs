...
namespace Project.V1.DLL.Crons
{
    public class MonthlyReportEmail : IJob, IDisposable
    {
        ...
        public MonthlyReportEmail(IRequest request, IVendor vendor, IProjectType projectType,
            ICLogger logger, UserManager<ApplicationUser> userManager, IScheduleJobRecipient IscheduleJobRecipient)
        {
            _request = request;
            _logger = logger;
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            try
            {
                var MonthlyRecipientsCSV = await ischeduleJobRecipient.GetByRecipientId("4258dd5e-9ec8-4be1-877b-120ba620c110");
                ...
                try
                {
                    ...
                    SendNotification(yesterDay, tableMonthlyAll, MonthlyRecipientsCSV.ToEmail, $"Tuning Acceptance {yesterDay:yyyy-MMMM}");
        ...
        private void SendVendorSpecificAccecptanceNotification(DateTime yesterDay, List<AcceptanceDTO> yesterdaysRequests)
        {
            ...
            var MonthlyDailyRecipientsData = ischeduleJobRecipient.GetByRecipientId("53acc3a8-5ab1-4a8c-a8dd-9021f5d1efde").GetAwaiter().GetResult();

            VendorRecipientsCSV.Add(MonthlyDailyRecipientsData.Name, MonthlyDailyRecipientsData.ToEmail);
        ...
        private async void SendNotification(DateTime yesterDay, string table, string recipients, string subject, bool isGeneral = true)
        {
            ...
            var recipientListCC = (ENV == "Development") ? devRecipient : HelperFunctions.ConvertMailStringToList(MonthlyRecipientsCSV.CCEmail);
           // var recipientListCC = (ENV == "Development") ? HelperFunctions.ConvertMailStringToList(MonthlyRecipientsCSV.CCEmail) : HelperFunctions.ConvertMailStringToList(MonthlyRecipientsCSV.CCEmail);

...
namespace Project.V1.DLL.Crons
{
    public class DailyReportEmail : IJob, IDisposable
    {
        ...
        private readonly IScheduleJobRecipient ischeduleJobRecipient;
        ...
        public string VendorName, VendorRecipietns;
        ...
        public DailyReportEmail(IRequest request, IVendor vendor, IProjectType projectType,
            ICLogger logger, IScheduleJobRecipient IscheduleJobRecipient)
        {
            _request = request;
            _logger = logger;
            ischeduleJobRecipient = IscheduleJobRecipient;
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            try
            {
                var DailyRecipientsCSV = await ischeduleJobRecipient.GetByRecipientId("fb74fb55-07c8-4387-9dee-118bf5a9712b");
                ...
                SendNotification(yesterDay, table, DailyRecipientsCSV.ToEmail, $"Tuning Acceptance {yesterDay:yyyy-MMMM-dd}");
        ...
        private void SendVendorSubmissionNotification(DateTime yesterDay, List<AcceptanceDTO> yesterdaysRequests)
        {
            ...
            var VendorDailyRecipientsData = ischeduleJobRecipient.GetByRecipientId("53acc3a8-5ab1-4a8c-a8dd-9021f5d1efde").GetAwaiter().GetResult();

            VendorRecipientsCSV.Add(VendorDailyRecipientsData.Name, VendorDailyRecipientsData.ToEmail);
            ...
            foreach (var vendorData in vendorYesterdayData)
            {
                ...
                if (VendorRecipientsCSV.ContainsKey(vendorData.Key.ToUpper()) && vendorData.Any())
                {
                    ...
                    SendNotification(yesterDay, vendorTables, VendorRecipientsCSV[vendorData.Key.ToUpper()], $"{vendorData.Key} Tuning Status {yesterDay:yyyy-MMMM-dd}", isGeneral: false);
                }
            }
        }
        ...
        private async void SendNotification(DateTime yesterDay, string table, string recipients, string subject, bool isGeneral = true)
        {
            var mvm = new MailerViewModel<DailyReportEmail>
            {
               ...
               // Greetings = $"<p>The following sites were accepted as at <strong>{yesterDay:dd.MM.yyyy}</strong>, by MTNN RF.</p>"
                Greetings = $"<p>The following sites have status below as at <strong>{yesterDay:dd.MM.yyyy}</strong>.</p>"
            };
            ...
            {
               // new SenderBody { Name = "", Address = "Anthony.Nwosu@mtn.com;Anthony.Nwosu@mtn.com" },
            ...
            // var recipientList = (ENV == "Development") ? HelperFunctions.ConvertMailStringToList(recipients) : HelperFunctions.ConvertMailStringToList(recipients);

            mvm.To = recipientList;

            var DailyRecipientsCSV = await ischeduleJobRecipient.GetByRecipientId("fb74fb55-07c8-4387-9dee-118bf5a9712b");

            var recipientListCC = (ENV == "Development") ? devRecipient : HelperFunctions.ConvertMailStringToList(DailyRecipientsCSV.CCEmail);
            //var recipientListCC = (ENV == "Development") ? HelperFunctions.ConvertMailStringToList(DailyRecipientsCSV.CCEmail) : HelperFunctions.ConvertMailStringToList(DailyRecipientsCSV.CCEmail);

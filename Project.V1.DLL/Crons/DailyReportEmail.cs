...
namespace Project.V1.DLL.Crons
{
    public class DailyReportEmail : IJob, IDisposable
    {
        ...
        public DailyReportEmail(IRequest request, IVendor vendor, IProjectType projectType, ICLogger logger)
        {
            _request = request;
            _logger = logger;
        ...
        private void SendVendorSpecificAccecptanceNotification(DateTime yesterDay, List<AcceptanceDTO> yesterdaysRequests)
        {
            ...
            foreach (var vendorData in vendorYesterdayData)
            {
                ...
                {
                    vendorTables += $"<p><b><br><br>{vendorData.Key} Sites accepted on {yesterDay:dd.MM.yyyy} are as follows: </b></p>";
        ...
        private async void SendNotification(DateTime yesterDay, string table, string recipients, string subject, bool isGeneral = true)
        {
            ...
            //var recipientList = (ENV == "Development") ? HelperFunctions.ConvertMailStringToList(recipients) : devRecipient;
